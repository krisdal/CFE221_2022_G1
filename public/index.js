...
async function getExchangeRate() {
    ...
    try {
        const response = await fetch(`https://v6.exchangerate-api.com/v6/24e6d660c286f4c452cf2818/latest/${fromCur.value}`);
...
document.querySelectorAll('a[href^="#"]').forEach(anchor => {
    anchor.addEventListener('click', function (e) {
        ...
        if (targetSection) {
            window.scrollTo({
                top: targetSection.offsetTop - 100, // Adjust scroll position if needed
                behavior: 'smooth'
            });
        }
    });
